...
def main():
    ...
    st.markdown("### DNA–RNA Based Image Encryption System")

    st.markdown(
        "Upload an image to securely encrypt and decrypt it using chaotic DNA–RNA encoding."
    )
    ...
    if uploaded_images:
        ...
        if st.button("Encrypt and Decrypt Images"):
            ...
            for i, img_file in enumerate(uploaded_images):
                ...
                img = Image.open(img_file).convert("RGB")  # Keep the image in RGB
            ...
            st.write("Processing Time:", round(end_time - start_time, 3), "seconds")
           ...
           # for i, (encrypted_img, decrypted_img) in enumerate(zip(encrypted_images, decrypted_images)):
            ...
            for img_file, encrypted_img, decrypted_img in zip(
                    uploaded_images, encrypted_images, decrypted_images):
